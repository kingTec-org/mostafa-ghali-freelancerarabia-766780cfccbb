                    ...
                    <ul class="navbar-nav ms-auto nav-menu-end">
                        <li class="nav-item">
                            <div class="dropdown dropright">
                                <button class="btn btn-primary font-xs nav_button_section_not_border dropdown-toggle" type="button" id="currency" data-bs-toggle="dropdown" aria-expanded="false">
                                   USD
...
"YER","ZAR","ZMW"] as $currency)
                                    <li><a class="dropdown-item" href="#" data-code="{{$currency}}">{{$currency}}</a></li>
                                    @endforeach
        ...
        <div class="navdownlist scrollmenu container-fluid mt-3 " style="display: none;">
            ...
            @foreach(@$original_categories??[] as $category)
                <a class="data-action-show-on-hover"
                   href="{{route('store.sub_category',$category->id)}}" data-action-show-on-hover="category-nav-items-{{$category->id}}">{{getLang() =='ar' ? $category->name_ar:$category->name_en}}</a>
...
@foreach(@$original_categories??[] as $category)
    @if($category->subCategories->count())
        ...
        <div class="second-nav bg-gray category-nav-items " style="display: none" id="category-nav-items-{{$category->id}}">
                ...
                <div class="container">
                    <div class="row w-100 ">
                        @foreach(@$category->subCategories??[] as $subcategory)
                            ...
                            <a class="col-md-3 nav-item" href="{{route('store.ServicesBaseSubCategory',$category->id)}}"> {{$subcategory['name_'.app()->getLocale()]}}</a>
                        @endforeach
